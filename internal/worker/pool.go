...
func NewPool(workerCount int, bufferSize int, s *store.Store, m *store.Metrics, exec executor.Executor) *Pool {
	return &Pool{
...
func (p *Pool) processOrder(order *models.Order) {
	...
	if execErr == nil {
		...
		log.Printf("Order %s COMPLETED", id)
		return
	}
